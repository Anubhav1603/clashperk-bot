...
export default class Resolver {
	...
	public async enforceSecurity(interaction: CommandInteraction<'cached'>, tag?: string) {
		if (!tag) {
			await interaction.editReply(i18next.t('common.no_clan_tag', { lng: interaction.locale }));
			return null;
		}
		...
		if (
			...
			await interaction.editReply({ content: i18next.t('common.clan_limit', { lng: interaction.locale }) });
			return null;
		}
