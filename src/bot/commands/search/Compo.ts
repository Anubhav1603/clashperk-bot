...
export default class CompoCommand extends Command {
	...
	public async exec(interaction: CommandInteraction<'cached'>, args: { tag?: string }) {
		...
		const avg =
			townHalls.reduce((p, c) => p + c.total * c.level, 0) / townHalls.reduce((p, c) => p + c.total, 0) || 0;
		...
		const embed = new EmbedBuilder()
			...
			.setDescription(
				townHalls.map((th) => `${TOWN_HALLS[th.level]} ${ORANGE_NUMBERS[th.total]}\u200b`).join('\n')
			)
			.setFooter({
		...
		this.client.logger.debug(`Executed in ${(diff[0] * 1000 + diff[1] / 1000000).toFixed(2)}ms`, {
			label: 'COMPO'
		});
